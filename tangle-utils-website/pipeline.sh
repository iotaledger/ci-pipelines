...
api_prod () {
  ...
  echo "    command:
      ...
      - jq -r '.zmqDevNet .endpoint = \\\$endpoint' --arg endpoint 'tcp://iri02.devnet.iota.cafe:5556' src/data/config.prod.json > tmp.json && mv tmp.json src/data/config.prod.json
