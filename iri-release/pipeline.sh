...
getMilestone () {
  ...
  echo "    commands:
      ...
      - docker run --rm --name iri -d -v /cache-iri-release-$BUILDKITE_BUILD_ID:/iri/data -p 14265:14265 iotaledger/iri-dev:$1
      - sleep 120
...
release () {
  ...
  echo "    commands:
      ...
      - curl -s https://iotaledger-iri-release.s3.eu-central-1.amazonaws.com/\\\$IRI_VERSION_NUMBER/iri-\\\$IRI_VERSION.jar --output target/IRI-\\\$IRI_VERSION.jar
      - curl -s https://iotaledger-iri-release.s3.eu-central-1.amazonaws.com/\\\$IRI_VERSION_NUMBER/SHA256SUM-\\\$IRI_VERSION --output target/IRI-\\\$IRI_VERSION-SHA256SUM
      ...
      - if [[ \\\$(sha256sum target/LS-\\\$(cat /cache/milestone.txt).tar | cut -d \" \" -f 1) == \\\$(cat target/LS-\\\$(cat /cache/milestone.txt)-SHA256SUM) ]]; then echo 'CHECKSUM OK'; else exit 1; fi
